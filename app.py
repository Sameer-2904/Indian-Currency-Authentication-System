# This file is a copy of the provided Streamlit app (Untitled-1.py).
# It's provided as `app.py` for convenient `streamlit run app.py` usage.
...
# Load custom CSS (if present) and render a branded header with logo
def _load_local_css(path: str):
    try:
...
with header_cols[0]:
    try:
        ...
        pass
with header_cols[1]:
    st.markdown("<h1 style='margin:0'>INDIAN CURRENCY AUTHENTICATION SYSTEM</h1>", unsafe_allow_html=True)
    st.markdown("<div style='color:#9fb7c7;margin-top:6px'>Detect genuine vs counterfeit Indian Rupee notes</div>", unsafe_allow_html=True)
